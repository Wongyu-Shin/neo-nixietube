...
9. **장소/장비는 "소유+접근" 하이브리드로 해결** — 전자레인지 미니가마+Arduino PID(자택, ₩71-101K, 프릿 소성), 메이커스페이스(레이저 커터), 네온사인 업체(진공 펌프+가스), 대학 실험실(화학 후드+진공). 경로 1(프릿)은 자택 소성 가능, 외부 접근은 로터리 펌프만 필요. 접근 실패 시 경로 2(상온 봉착)로 즉시 시작 가능
10. **경로 2의 전략적 의미** — 단순히 저렴한 대안이 아니라, 장비 미확보 상태에서 즉시 시작 가능한 유일한 경로. 경로 2로 PoC 진행하면서 경로 1 장비 접근 구축이 최적 전략
